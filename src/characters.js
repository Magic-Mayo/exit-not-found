...
const Character = function(name, clas){
    ...
    this.addStat = function(stat){
        ...
        switch(stat){
            case 0: return this.attack = roundUp(this.attack * 1.2);
            case 1: return this.agility = roundUp(this.agility * 1.1);
            case 2: return this.def = roundUp(this.def * 1.15);
            case 3: return this.speed = roundUp(this.speed * 1.1)
        }
    };
    ...
    this.attackEnemy = function(enemy){
        ...
        return willHit ? `You hit for ${this.attack - enemy.def}!` : `${this.name} missed!`;
    };
}
...
const Enemy = function(difficulty){
    ...
    this.atkChar = function(char){
        ...
        return char.def >= this.attack ? `You blocked ${this.name}'s attack!` : willHit ? `${this.name} hit for ${this.attack}!` : `${this.name} missed!`;
    }
    this.move = function(){
...
//         const room = new RoomBuilder([rng(), rng()]);
//         room.addCells();
//         rooms.push(room);
//     }
// }
