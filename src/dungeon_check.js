...
const checker = (room, [cX, cY], [pX, pY], [eX, eY], [sX, sY], direction, tileSize, [xMax, yMax])=>{
  ...
  ------------------------------------------------`);
  const move = (x, y, dir = direction, xX = cX, yY = cY) => setTimeout(()=>checker(room, [x, y], [xX, yY], [eX, eY], [sX, sY], dir, tileSize, [xMax, yMax]),250);
  const back = ([x,y], dir, j=1) => {
    console.log('back')
    if(!dir){
      if(room[x][y+tileSize] && room[x+tileSize][y+tileSize])return move(x+tileSize,y+tileSize, 0, x,y + tileSize)
    ...
    if(dir==2){
      if(room[x][y-tileSize] && room[x-tileSize][y-tileSize])return move(x-tileSize,y-tileSize, 2, x,y-tileSize)
      // if(room[x][y-tileSize*j] && room[x-tileSize][y-tileSize*j])
  ...
  if(((cX != pX || cY != pY) && cX == sX && cY == sY) ||
    (!upOneAvail && !rightOneAvail && !leftOneAvail && !downOneAvail))return false;
  ...
  if(!direction){
    ...
    if(downOneAvail && (movedLeft || (movedUp && sameX)))return move(cX, cY + tileSize);
  ...
  if(direction == 1){
    ...
    if(upOneAvail && ((cY && sameY && movedRight && pY != cY - tileSize) || movedDown))return move(cX, cY - tileSize);

    
    // if we just moved up check to see if we can move left,
    // if so move left or if we just moved left see if we can again
    if(leftOneAvail && (movedDown || (movedLeft && sameY)))return move(cX - tileSize, cY);
  ...
  if(direction == 2){
    ...
    if(rightOneAvail && ((cX != xMax && sameX && movedDown && pX != cX + tileSize) || movedRight))return move(cX + tileSize, cY);
    
    // if we just moved right check to see if we can move up,
    // if so move up or if we just moved up do it again if we can
    if(upOneAvail && (movedRight || (movedDown && sameX)))return move(cX, cY - tileSize);
  ...
  if(downOneAvail && ((cY != yMax && sameY && movedLeft && pY != cY + tileSize) || movedUp))return move(cX, cY + tileSize);
  
  // if we just moved down check to see if we can move right
  // and do it or if we just moved right do it again if we can
  if(rightOneAvail && (movedUp || (movedRight && sameY)))return move(cX + tileSize, cY);
