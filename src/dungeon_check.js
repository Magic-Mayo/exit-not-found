...
const checker = (
  ...
  ctx.fillStyle = room[pX][pY] && "green";
  ctx.fillRect(pX, pY, tileSize, tileSize);
  ctx.fillStyle = room[cX][cY] && "lightblue";
  ...
  const move = (x, y, dir = direction, xX = cX, yY = cY) =>
    setTimeout(() => checker(room, [x, y], [xX, yY], [eX, eY], [sX, sY], dir, tileSize, [
      xMax,
      yMax,
    ]), 100);
  const back = ([x, y], dir, j = 1) => {
    if (!dir) {
      if (room[x][y + tileSize] && room[x + tileSize][y + tileSize])
        return move(x + tileSize, y + tileSize, 0, x + tileSize, y);
    ...
    if (dir == 2) {
      if (room[x][y - tileSize] && room[x - tileSize][y - tileSize])
        return move(x - tileSize, y - tileSize, 2, x - tileSize, y);
      if (room[x][y - tileSize * j] && room[x - tileSize][y - tileSize * j])
  ...
  if (
    ...
    (!upOne && !rightOne && !leftOne && !downOne)
  )
    return false;
  ...
  if (!direction) {
    ...
    if (leftOne && cX && cX == pX && cY <= pY - tileSize && pX != cX - tileSize)
  ...
  if (direction == 1) {
    ...
    if (upOne && cY && cY == pY && cX >= pX + tileSize && pY != cY - tileSize)
